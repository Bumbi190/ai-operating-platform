/**
 * `/approvals` before vNext — the Granskningscenter body, moved here verbatim.
 *
 * Reachable at `?ui=legacy` (or with the `omnira_ui=legacy` cookie). Nothing in
 * it changed in the move: the same scoped read, the same ApprovalCard, the same
 * layout, so the rollback is the page that shipped, not a re-creation of it.
 */
...
export async function ApprovalsLegacy() {
