...
const QUEUE_META: Record<ReviewQueue, { dot: string; label: string }> = {
  ...
  needs_input: { dot: 'bg-muted-foreground/50', label: 'Behöver underlag' },
}
...
function ScoreBadge({ score, verdict, critical, status }: { score: number | null; verdict: string | null; critical: boolean; status: string }) {
  if (status === 'drafted' || score == null) {
    return <span className="inline-flex items-center gap-1 rounded-full bg-muted px-2.5 py-1 text-xs font-semibold text-muted-foreground"><Loader2 className="h-3.5 w-3.5 animate-spin" />Guard körs…</span>
  }
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {data.months.map((m) => (
          <span key={m.plan_key} className="inline-flex items-center gap-1.5 rounded-lg border bg-card px-3 py-1.5 text-sm">
            <span className="font-semibold">{m.label}</span>
            <span className="text-muted-foreground">{m.theme_name ?? 'tema ej satt'}</span>
          </span>
        ))}
      </div>
      ...
      <div className="mb-5 flex flex-wrap gap-2">
        {chips.map(({ key, n }) => (
          <button key={key} onClick={() => setFilter(filter === key ? 'all' : key)}
            className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm transition ${filter === key ? 'border-primary bg-primary text-primary-foreground' : 'bg-card hover:bg-muted/60'} ${n === 0 && filter !== key ? 'opacity-40' : ''}`}>
      ...
      {visible.length === 0 ? (
        (filter === 'pending' || filter === 'all') ? (
          <div className="rounded-xl border border-dashed bg-card/60 px-6 py-14 text-center">
            ...
            <p className="text-base font-semibold text-foreground">Allt granskat ✓</p>
            <p className="mt-1 text-sm text-muted-foreground">Inget väntar på beslut just nu.</p>
          </div>
        ) : (
          <div className="rounded-xl border border-dashed bg-card/60 px-6 py-12 text-center text-muted-foreground">
    ...
    <div className="overflow-hidden rounded-xl border bg-card shadow-sm transition hover:shadow-md">
      ...
      <button onClick={onToggle} className="flex w-full items-start gap-3 px-4 py-3.5 text-left">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{card.caption_preview || <span className="italic text-muted-foreground/60">Ingen caption</span>}{card.caption_full.length > 100 ? '…' : ''}</p>
          {card.primary_reason && (
      ...
      <div className="flex items-center gap-2 border-t bg-muted/40/60 px-4 py-2.5">
        {primary === 'approve' && (
        ...
        {primary === 'fix' && (
          <button onClick={props.onEditStart}
            className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3.5 py-1.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90">
            <Wrench className="h-4 w-4" />Åtgärda
          </button>
        )}
        {primary === 'return' && (
          <button disabled={isBusy('return')} onClick={() => onAct(card.draft_id, 'return')}
            className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-card px-3.5 py-1.5 text-sm font-semibold hover:bg-muted/60 disabled:opacity-50">
        ...
        <button onClick={onMenuToggle} aria-label="Fler val" aria-expanded={menuOpen}
          className={`ml-auto inline-flex items-center rounded-lg border px-2 py-1.5 text-muted-foreground hover:bg-muted ${menuOpen ? 'bg-card' : ''}`}>
      ...
      {menuOpen && (
        <div className="flex flex-wrap gap-1.5 border-t bg-card px-4 py-2">
          ...
          {editing ? (
            ...
            <>
              <Section title="Caption">
                <p className="whitespace-pre-wrap text-foreground">{card.caption_full || '—'}</p>
              </Section>
                ...
                <Section title="Problem">
                  <ul className="space-y-1">
                    {card.violations.map((v, i) => {
                      ...
                      return <li key={i} className="flex gap-2"><span className={`mt-0.5 rounded px-1.5 py-0.5 text-[10px] font-bold ${v.severity === 'CRITICAL' ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}>{sev}</span><span className="text-foreground">{v.explanation}</span></li>
                    })}
                  </ul>
                </Section>
              )}
              <Section title="CTA">
                <span className="rounded bg-muted px-2 py-0.5">{card.cta.label ?? '—'}</span>
              ...
              <details className="mt-1 rounded-lg bg-muted/40 px-3 py-2">
                <summary className="cursor-pointer text-xs font-semibold text-muted-foreground">Bilder</summary>
                <ul className="mt-2 space-y-0.5">
                  {card.asset_refs.length === 0 ? <li className="text-muted-foreground/60">—</li> : card.asset_refs.map((a, i) => (
                    <li key={i} className="flex items-center gap-2 text-xs">
                      <span className="text-foreground">{a.ref ?? '(ingen)'}</span>
              ...
              {/* Audit-tidslinje */}
              <details className="mt-3 rounded-lg bg-muted/40 px-3 py-2">
                <summary className="cursor-pointer text-xs font-semibold text-muted-foreground">Tidslinje</summary>
                <ol className="mt-2 space-y-2">
                  {card.audit.map((step, i) => (
                    <li key={i} className="flex items-center gap-2 text-xs">
                      <span className={`h-2 w-2 rounded-full ${step.at ? 'bg-emerald-500' : 'bg-muted-foreground/30'}`} />
                      <span className="font-medium text-foreground">{step.label}</span>
                      <Clock className="h-3 w-3 text-muted-foreground/60" /><span className="text-muted-foreground">{rel(step.at)}</span>
                    </li>
                  ))}
                </ol>
              </details>
              ...
              <details className="mt-2 rounded-lg bg-muted/40 px-3 py-2">
                <summary className="cursor-pointer text-xs font-semibold text-muted-foreground">Tekniska detaljer</summary>
...
function MenuItem({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick}
      className="inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-medium text-foreground hover:bg-muted/60">
      {icon}{label}
    </button>
  )
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-3">
      <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground/60">{title}</div>
      <div>{children}</div>
    </div>
  )
}
...
function EditPanel(props: {
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-muted-foreground/60">Caption</label>
        <textarea value={editCaption} onChange={(e) => setEditCaption(e.target.value)} rows={5}
          className="w-full rounded-lg border px-3 py-2 text-sm" />
      </div>
      {needsUrl && (
        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-muted-foreground/60">Landningssida (UTM-URL)</label>
      ...
      <p className="text-xs text-muted-foreground/60">Sparar ändringen och kör Guard igen så poängen uppdateras.</p>
    </div>
  )
}
