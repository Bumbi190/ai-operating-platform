...
/**
 * LEGACY Planning — the rollback path, and ONLY the rollback path.
 *
 * This is the pre-vNext surface, moved here unchanged so `?ui=legacy` keeps
 * working. It is NOT the source of truth for what Planning is: its hero copy,
 * its roadmap and its board contents are hard-coded, and `PlanningBoard`'s
 * add / drag / delete controls mutate local state only — nothing it shows is
 * read from the database and nothing it does is persisted.
 *
 * vNext Planning replaces all of it with reads of `manager_tasks`, cron-
 * triggered `workflows` and `workflow_instances`. See `PlanningView`.
 *
 * Deliberately left alone: no new behaviour is added to legacy, and the
 * fabricated interactions are not carried into vNext.
 */
export function PlanningLegacy() {
