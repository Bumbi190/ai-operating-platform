...
/**
 * Legacy project body — the `?ui=legacy` rollback for `/projects/[slug]`.
 *
 * Moved VERBATIM out of `page.tsx` in vNext Phase 10: same imports, same reads,
 * same markup. Only its entry changed — the page now resolves the project (and
 * 404s) before choosing a generation, and hands the resolved project here, so
 * this body renders exactly what the route rendered before. Do not add vNext
 * behaviour to it; the rollback must stay a rollback.
 */
export async function ProjectLegacy({
  slug,
  project,
}: {
  slug: string
