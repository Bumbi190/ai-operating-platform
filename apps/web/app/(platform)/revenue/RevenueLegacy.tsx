/**
 * `/revenue` — the legacy Revenue Center body, moved verbatim.
 *
 * This file is the rollback target for `?ui=legacy`. Nothing in it was
 * rewritten: the body below is the previous page's body byte-for-byte, with
 * exactly two mechanical edits — the segment config moved to `page.tsx`, which
 * owns it for both generations, and the default export became a named one so
 * the branch can render it. The tests pin that with a hash.
 *
 * `CostIntelligence.tsx` and `LiveRefresh.tsx` stay where they are and are
 * imported unchanged; they are part of this body, not of vNext.
 *
 * Do not improve anything here — including the figures Phase 14 found to be
 * unsupported. A rollback that renders something other than what it replaced
 * is not a rollback.
 */
...
export async function RevenueLegacy() {
