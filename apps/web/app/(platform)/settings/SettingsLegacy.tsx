/**
 * `/settings` before vNext — the Inställningar body, moved here verbatim.
 *
 * Reachable at `?ui=legacy` (or with the `omnira_ui=legacy` cookie). One edit was
 * made in the move: the default export became a named one so the branch can
 * render it. Everything else is the page that shipped — the account panel with
 * its "Magic link (e-post)" line, the key presence list that points at
 * `.env.local`, the display preferences, the Instagram/Facebook TokenUpdater,
 * the hard-coded version, stack and model, and the "Kommande funktioner" hint.
 * vNext Settings deliberately carries none of those claims forward. Settings S0
 * had already removed the seed control from this body.
 *
 * Do not improve anything here. A rollback that renders something other than
 * what it replaced is not a rollback.
 */
...
export async function SettingsLegacy() {
