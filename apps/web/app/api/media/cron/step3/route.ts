...
export async function GET(request: Request) {
  ...
  const { data: claimed, error: claimError } = await (db as any)
    .rpc('claim_media_script_for_render', { p_project_id: projectIdParam, p_script_id: scriptIdParam })
  if (claimError) {
  ...
  if (!claim || claim.status === 'nothing_eligible') {
    return NextResponse.json({ status: 'nothing_to_do', reason: claim?.reason ?? 'No script claim available for render' })
  }
  if (claim.status !== 'claimed' && claim.status !== 'stale_claim_recovered') {
    return NextResponse.json({ status: 'render_not_claimed', claim }, { status: claim.status === 'project_mismatch' ? 403 : 409 })
  }
  ...
  try {
    ...
    await assertMediaProductionEligible(db, { projectId, scriptId: script.id, stage: 'render' })
