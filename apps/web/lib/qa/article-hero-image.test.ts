...
// G3C-1: the legacy checkAutomationPaused preflight was replaced by the
// canonical early eligibility check. `automationPause` still drives it, so the
// existing "paused" cases keep working — they now exercise the canonical
// authority instead of the legacy helper.
vi.mock('@/lib/governance/execution-preflight', () => ({
  resolveExecutionEligibility: async () => (
    automationPause.allowed
      ? { allowed: true, reason: null }
      : { allowed: false, reason: 'global_automation_paused' }
  ),
}))
...
describe('generateHeroImage — MVP Commit 3 + Hero Image V2 shadow', () => {
  ...
  it('4. automation paused: returns skipped with operator reason, no work claimed', async () => {
    ...
    if (!result.ok) {
      expect(result.status).toBe('skipped')
      // G3C-1: the canonical authority returns a STABLE code from a closed
        // vocabulary, not the legacy helper's localized sentence.
        expect(result.reason).toBe('global_automation_paused')
    }
    expect(ideogramCalls).toHaveLength(0)
