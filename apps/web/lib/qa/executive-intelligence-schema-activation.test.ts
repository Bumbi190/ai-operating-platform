...
describe('EI-S1.6A — Executive Intelligence schema activation bundle', () => {
  ...
  it('keeps the legacy EI companion count at exactly 83 enforced migrations', () => {
    ...
    // the count is pinned rather than derived precisely so two concurrent
    // migrations cannot both look reviewed while only one was counted.
    //
    // 67: Phase 9Y's `workflow_stories_rls`. The tripwire caught it on the first
    // full run, which is the point — and there is some irony in where it landed.
    // This suite exists because three migrations sat in the repo-root directory
    // that the guard did not scan; Phase 9Y's finding was a table whose creation
    ...
    // this counter guards.
    //
    // 69: Phase 9AB's `cost_ledger_rls_isolation` — five policies whose
    // `project_id IS NULL OR …` branch let anon into platform-level cost rows.
    //
    // 70: Phase 9AA's `agent_scorecards_view_isolation` — a view that ran as its
    ...
    //
    // 74–78: project-scoped social credentials (owner decisions 1A · 2Y1 · 3O1) —
    // `social_account_bindings` (Project → Platform → Verified External Account, one
    // project per account), `platform_credential_events_account_binding` (the audit
    // names the attested account; event_version 2 makes a pre-v2 writer refuse),
    // `social_credential_health` (per-project verification, closed codes only),
    ...
    // 83: Phase 1C1A's `sdf1c1_trusted_broker_identity` — device identity and
