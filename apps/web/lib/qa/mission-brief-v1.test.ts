...
const COMPLETE_BRIEF = {
  ...
  dependencies: [{ kind: 'capability' as const, reference: 'isolation-primitives', hardness: 'hard' as const, satisfied: true }],
  ...
  approvalGates: [{ gate: 'Before publishing' }],
  deadline: DEADLINE,
...
async function proposedMission(store = new FakeStore()): Promise<{ store: FakeStore; missionId: string }> {
  ...
  const opened = await openMission({ projectId: PROJECT_P, ...COMPLETE_BRIEF, store, now: T0 })
...
async function approvedMission(): Promise<{ store: FakeStore; missionId: string; grant: Grant }> {
  ...
  const grant = await grantFor({ act: 'approve', missionId, store, now: T1 }, AUTH_A)
  authorizationService([grant])
  const approved = await approveMission({ missionId, authorizationId: AUTH_A, store, now: T1 })
...
async function activatedMission(): Promise<{ store: FakeStore; missionId: string; grants: Grant[] }> {
  ...
  const activateGrant = await grantFor({ act: 'activate', missionId, store, now: T2 }, AUTH_B)
  authorizationService([grant, activateGrant])
  const activated = await activateMission({ missionId, authorizationId: AUTH_B, store, now: T2 })
  expect(`${activated.status}${activated.detail ? `:${activated.detail}` : ''}`).toBe('ok')
  return { store, missionId, grants: [grant, activateGrant] }
}

beforeEach(() => { mockAccess.mockReset(); mockAuth.mockReset(); mockDecision.mockReset() })
...
describe('Opening a mission grants nothing (§20.99)', () => {
  it('creates a draft that is not approved, not active and not authorized', async () => {
    ...
    const opened = await openMission({ projectId: PROJECT_P, ...COMPLETE_BRIEF, asDraft: true, store, now: T0 })
    expect(opened.status).toBe('ok')
    ...
    const authority = await isMissionOperationallyAuthorized(opened.state!.missionId, { store, now: T1 })
  ...
  it('creates a proposal and derives awaiting_approval once the brief is complete', async () => {
    const { store, missionId } = await proposedMission()
    const read = await resolveMission(missionId, { store, now: T1 })
  ...
  it('keeps an incomplete proposal at `proposed`, naming what is missing (§20.172)', async () => {
    ...
    const opened = await openMission({ projectId: PROJECT_P, ...thin, store, now: T0 })
  ...
  it('refuses to approve an incomplete brief without appending anything', async () => {
    ...
    const result = await approveMission({ missionId: opened.state!.missionId, authorizationId: AUTH_A, store, now: T1 })
...
describe('Only an exact, current authorization approves a mission', () => {
  it('accepts the exact approved candidate that was authorized', async () => {
    ...
    const read = await resolveMission(missionId, { store, now: T2 })
  ...
  it('refuses an unrelated same-project grant', async () => {
    ...
    const before = store.appended.length
    const result = await approveMission({ missionId, authorizationId: AUTH_UNRELATED, store, now: T1 })
  ...
  it('refuses a wrong-project authorization', async () => {
    const { store, missionId } = await proposedMission()
    const grant = await grantFor({ act: 'approve', missionId, store, now: T1 }, AUTH_A, { projectId: PROJECT_Q })
    authorizationService([grant])
    const result = await approveMission({ missionId, authorizationId: AUTH_A, store, now: T1 })
    expect(result.status).toBe('authority_not_effective')
  ...
  it('refuses when the authorization principal is not the acting caller (§20.55)', async () => {
    const { store, missionId } = await proposedMission()
    const grant = await grantFor({ act: 'approve', missionId, store, now: T1 }, AUTH_A, { principalId: PRINCIPAL_B })
    authorizationService([grant])
    const before = store.appended.length
    const result = await approveMission({ missionId, authorizationId: AUTH_A, store, now: T1 })
  ...
  it('refuses a conditional-unverified authorization', async () => {
    const { store, missionId } = await proposedMission()
    const grant = await grantFor({ act: 'approve', missionId, store, now: T1 }, AUTH_A, { conditionsUnverified: true })
    authorizationService([grant])
    const result = await approveMission({ missionId, authorizationId: AUTH_A, store, now: T1 })
    expect(result.status).toBe('authority_not_effective')
...
describe('Mission authority is an operational gate, not a historical fact (§20.75)', () => {
  it('reports authorized while the grant is effective', async () => {
    const { store, missionId } = await approvedMission()
    const authority = await isMissionOperationallyAuthorized(missionId, { store, now: T2 })
  ...
  it('stops authorizing once the grant expires — unlike a Decision Ledger approval', async () => {
    const { store, missionId } = await proposedMission()
    const grant = await grantFor({ act: 'approve', missionId, store, now: T1 }, AUTH_A, { expiresAt: T2 })
    authorizationService([grant])
    expect((await approveMission({ missionId, authorizationId: AUTH_A, store, now: T1 })).status).toBe('ok')

    const authority = await isMissionOperationallyAuthorized(missionId, { store, now: T3 })
    expect(authority.authorized).toBe(false)
    ...
    const read = await resolveMission(missionId, { store, now: T3 })
    expect(read.state?.approvedAt).toBe(T1)
  ...
  it('blocks a NEW activation once authority is no longer valid', async () => {
    ...
    const before = store.appended.length
    const result = await activateMission({ missionId, authorizationId: AUTH_B, store, now: T2 })
    expect(result.status).toBe('authority_not_effective')
    expect(store.appended).toHaveLength(before)
  })
  ...
  it('makes an active mission non-ready when its authority later fails', async () => {
    ...
    const readiness = await resolveMissionReadiness(missionId, { store, now: T3 })
...
describe('Material amendment creates version N+1 and expires prior approval (§20.126, §20.75)', () => {
  ...
  it('creates N+1 under fresh exact authority and keeps N auditable', async () => {
    const { store, missionId } = await approvedMission()
    const amendGrant = await grantFor({ act: 'amend', missionId, ...AMENDMENT, store, now: T2 }, AUTH_B)
    authorizationService([amendGrant])
    const result = await amendMission({ missionId, authorizationId: AUTH_B, ...AMENDMENT, store, now: T2 })
  ...
  it('returns the mission to `proposed` — the prior approval expired (§20.75)', async () => {
    ...
    const amendGrant = await grantFor({ act: 'amend', missionId, ...AMENDMENT, store, now: T2 }, AUTH_B)
    authorizationService([amendGrant])
    await amendMission({ missionId, authorizationId: AUTH_B, ...AMENDMENT, store, now: T2 })
  ...
  it('refuses an amendment carrying only the approval authority', async () => {
    const { store, missionId } = await approvedMission()
    const before = store.appended.length
    const result = await amendMission({ missionId, authorizationId: AUTH_A, ...AMENDMENT, store, now: T2 })
    expect(result.status).toBe('authority_not_effective')
    expect(store.appended).toHaveLength(before)
  })
...
describe('Approved, Ready and Active are distinct (§20.100, §20.101, §20.102)', () => {
  it('an approved mission with an unmet hard dependency is approved but not ready', async () => {
    ...
    const opened = await openMission({ projectId: PROJECT_P, ...brief, store, now: T0 })
    const missionId = opened.state!.missionId
    const grant = await grantFor({ act: 'approve', missionId, store, now: T1 }, AUTH_A)
    authorizationService([grant])
    expect((await approveMission({ missionId, authorizationId: AUTH_A, store, now: T1 })).status).toBe('ok')

    const readiness = await resolveMissionReadiness(missionId, { store, now: T1 })
  ...
  it('refuses activation with missing requirements and appends nothing (§20.106)', async () => {
    ...
    const grant = await grantFor({ act: 'approve', missionId, store, now: T1 }, AUTH_A)
    authorizationService([grant])
    await approveMission({ missionId, authorizationId: AUTH_A, store, now: T1 })
...
describe('No invalid transition ever reaches the store', () => {
  it.each([
    ['approve a draft that was never proposed', async (store: FakeStore, id: string) =>
      approveMission({ missionId: id, authorizationId: AUTH_A, store, now: T1 })],
    ['resume a mission that is not paused', async (store: FakeStore, id: string) =>
      resumeMission({ missionId: id, store, now: T2 })],
    ['archive a mission that is not closed', async (store: FakeStore, id: string) =>
      archiveMission({ missionId: id, store, now: T2 })],
  ...
  it('refuses to close an active mission that has not met its own contract', async () => {
    const { store, missionId } = await activatedMission()
    const before = store.appended.length
    const result = await closeMission({ missionId, closure: CLOSURE, store, now: T3 })
...
describe('Completion is judged, never assumed (§20.92, §20.93, §20.225)', () => {
  async function readyToClose() {
    const { store, missionId } = await activatedMission()
    await recordMissionEvidence({ missionId, evidence: EVIDENCE, store, now: T3 })
    await reviewMission({ missionId, reviewNote: 'Objective achieved; criteria met; evidence sufficient.', store, now: T3 })
  ...
  it('completes once evidence, minimum criteria and a review all exist', async () => {
    ...
    const result = await closeMission({ missionId, closure: CLOSURE, store, now: T4 })
    expect(`${result.status}${result.detail ? `:${result.detail}` : ''}`).toBe('ok')
  ...
  it('distinguishes partial completion from completion (§20.94)', async () => {
    const { store, missionId } = await readyToClose()
    const result = await closeMission({ missionId, closure: CLOSURE, partial: true, store, now: T4 })
  ...
  it('distinguishes failure, which needs no completion evidence (§20.95)', async () => {
    const { store, missionId } = await activatedMission()
    const result = await failMission({ missionId, reason: 'Critical assumption failed.', store, now: T3 })
    expect(result.status).toBe('ok')
  ...
  it('distinguishes cancellation, which is an authority act (§20.96)', async () => {
    const { store, missionId } = await approvedMission()
    const noGrant = await cancelMission({ missionId, authorizationId: AUTH_A, reason: 'Priority changed.', store, now: T2 })
    expect(noGrant.status).toBe('authority_not_effective')

    const grant = await grantFor({ act: 'cancel', missionId, reason: 'Priority changed.', store, now: T2 }, AUTH_B)
    authorizationService([grant])
    const result = await cancelMission({ missionId, authorizationId: AUTH_B, reason: 'Priority changed.', store, now: T2 })
    expect(result.status).toBe('ok')
    expect(result.state?.status).toBe('cancelled')
  })

  it('supersedes with an explicit successor and preserves history (§20.97)', async () => {
    const { store, missionId } = await approvedMission()
    const successor = newMissionId()
    const grant = await grantFor({ act: 'supersede', missionId, supersededBy: successor, store, now: T2 }, AUTH_B)
    authorizationService([grant])
    const result = await supersedeMission({ missionId, supersededBy: successor, authorizationId: AUTH_B, store, now: T2 })
    expect(result.status).toBe('ok')
    ...
    const self = await supersedeMission({ missionId, supersededBy: missionId, authorizationId: AUTH_B, store, now: T3 })
    expect(self.status).toBe('invalid_request')
  })
  ...
  it('writes nothing to the Decision Ledger, Memory or any calibration surface', () => {
    ...
    // The read boundary may READ a decision; it must never write one.
    const read = readFileSync(resolve(MISSION_DIR, 'principal-read.ts'), 'utf8')
    expect(read).toContain('resolveDecision')
    expect(read).not.toMatch(/decision-ledger\/principal-write/)
  })
})
...
describe('Decision Ledger linkage is optional and validated (§20.137)', () => {
  ...
  it.each([['expired'], ['reversed'], ['superseded']])(
    'blocks operational authority when the governing decision is %s', async (status) => {
      ...
      const store = new FakeStore()
      const opened = await openMission({ projectId: PROJECT_P, ...withDecision, store, now: T0 })
      const missionId = opened.state!.missionId
      const grant = await grantFor({ act: 'approve', missionId, store, now: T1 }, AUTH_A)
      authorizationService([grant])
      expect((await approveMission({ missionId, authorizationId: AUTH_A, store, now: T1 })).status).toBe('ok')
...
describe('Project isolation (§6.117, §20.27)', () => {
  it('refuses to create a mission in a project the principal does not own', async () => {
    ...
    authorizationService([])
    const store = new FakeStore()
    const result = await openMission({ projectId: PROJECT_Q, ...COMPLETE_BRIEF, store, now: T0 })
  ...
  it('refuses to write to a foreign-project mission', async () => {
    ...
    authorizationService([])
    const before = store.appended.length
    const result = await pauseMission({ missionId, reason: 'x', store, now: T2 })
  ...
  it('makes unknown and foreign indistinguishable — no existence oracle', async () => {
    const { store, missionId } = await approvedMission()
    authAs(PRINCIPAL_B, [PROJECT_Q])
    const foreign = await resolveMission(missionId, { store, now: T2 })
    const unknown = await resolveMission(newMissionId(), { store, now: T2 })
...
describe('Lifecycle concurrency and deterministic ordering', () => {
  it('counts lifecycle acts, not rows, and excludes annotations', async () => {
    ...
    await reportMissionProgress({ missionId, report: { summary: 'Going well', atRisk: false }, store, now: T3 })
    await recordMissionEvidence({ missionId, evidence: EVIDENCE, store, now: T3 })
    await reviewMission({ missionId, reviewNote: 'Looks right', store, now: T3 })
  ...
  it('refuses the losing writer when an annotation interleaves two lifecycle acts', async () => {
    ...
    await reportMissionProgress({ missionId, report: { summary: 'Note', atRisk: false }, store, now: T3 })
    expect((await store.lineage(missionId))).toHaveLength(4)

    // Writer B pauses first, from the live lineage.
    authorizationService(grants)
    expect((await pauseMission({ missionId, reason: 'Founder review pending.', store, now: T3 })).status).toBe('ok')

    // Writer A now commits against its stale snapshot — and loses.
    const result = await failMission({ missionId, reason: 'Assumption failed.', store: aView, now: T3 })
  ...
  it('lets two annotations coexist at the same generation', async () => {
    const { store, missionId } = await activatedMission()
    expect((await reportMissionProgress({ missionId, report: { summary: 'one', atRisk: false }, store, now: T3 })).status).toBe('ok')
    expect((await reportMissionProgress({ missionId, report: { summary: 'two', atRisk: false }, store, now: T3 })).status).toBe('ok')
...
describe('Derived predicates and the no-execution boundary', () => {
  it('derives blocked from an explicit unresolved blocker (§20.103, §20.87)', async () => {
    const { store, missionId } = await activatedMission()
    await raiseMissionBlocker({ missionId, blocker: { blockerId: 'b-1', reason: 'Provider unstable' }, store, now: T3 })
    expect((await resolveMission(missionId, { store, now: T3 })).state?.status).toBe('blocked')

    await clearMissionBlocker({ missionId, blockerId: 'b-1', store, now: T4 })
    expect((await resolveMission(missionId, { store, now: T4 })).state?.status).toBe('active')
  })
  ...
  it('derives at_risk once the deadline has passed (§20.104)', async () => {
    ...
    authorizationService([])
    const store = new FakeStore()
    const opened = await openMission({ projectId: PROJECT_P, ...COMPLETE_BRIEF, deadline: PAST_DEADLINE, store, now: T0 })
    const missionId = opened.state!.missionId
    const approveGrant = await grantFor({ act: 'approve', missionId, store, now: T1 }, AUTH_A)
    authorizationService([approveGrant])
    await approveMission({ missionId, authorizationId: AUTH_A, store, now: T1 })
    const activateGrant = await grantFor({ act: 'activate', missionId, store, now: T2 }, AUTH_B)
    authorizationService([approveGrant, activateGrant])
    await activateMission({ missionId, authorizationId: AUTH_B, store, now: T2 })

    // Evidence not yet in, so awaiting_review does not pre-empt at_risk.
    expect((await resolveMission(missionId, { store, now: T3 })).state?.status).toBe('at_risk')
  })

  it('derives awaiting_review once evidence is in and no review exists (§20.195)', async () => {
    const { store, missionId } = await activatedMission()
    await recordMissionEvidence({ missionId, evidence: EVIDENCE, store, now: T3 })
    expect((await resolveMission(missionId, { store, now: T3 })).state?.status).toBe('awaiting_review')
    await reviewMission({ missionId, reviewNote: 'Reviewed.', store, now: T3 })
    expect((await resolveMission(missionId, { store, now: T3 })).state?.status).toBe('active')
  })

  it('pauses and resumes without an authority act (§20.132, §20.133)', async () => {
    const { store, missionId } = await activatedMission()
    authorizationService([])
    expect((await pauseMission({ missionId, reason: 'Founder review pending.', store, now: T3 })).state?.status).toBe('paused')
    expect((await resumeMission({ missionId, store, now: T4 })).state?.status).toBe('active')
  })
