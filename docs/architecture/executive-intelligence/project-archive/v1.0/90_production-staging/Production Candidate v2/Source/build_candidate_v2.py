...
def from_cover(c):
    ...
    c.drawCentredString(W/2,1.7*72,"PROFESSIONAL EDITION · VERSION 1.0")
    c.setFillColor(col(GHOST)); c.setFont("Mono",8)
    c.drawCentredString(W/2,1.35*72,"PRODUCTION CANDIDATE v2 — NOT FINAL RELEASE")
def from_title(c):
    c.setFillColor(col(DARK)); c.rect(0,0,W,H,fill=1,stroke=0)
    ...
    rows=[("Title","Omnira — Executive Intelligence"),
          ...
          ("Build status","PRODUCTION CANDIDATE v2 — not final professional release"),
...
def from_chapopen(c, ex):
    ...
    for p in ex.get("paras",[]):
        ...
        if yy<1.15*72: break
    c.setFillColor(col(GHOST)); c.setFont("Mono",7.5)
    c.drawString(1.2*72,0.92*72,"Opening excerpt is verbatim canonical text — no boilerplate, no rewrite.")

def set_meta(c):
    c.setTitle("Omnira — Executive Intelligence — Professional Edition Candidate v2 (PRODUCTION CANDIDATE — not final release)")
    c.setAuthor("André Hultgren")
    c.setSubject("Canonical Architecture and Operating Doctrine — Professional Edition PRODUCTION CANDIDATE v2")
    c.setKeywords("Omnira, Executive Intelligence, Professional Edition, PRODUCTION CANDIDATE v2, not final release, Canonical v1.0, Layout C")
    c.setCreator("Omnira Publishing Systems — build_candidate_v2.py (ReportLab)")
...
def main():
    ...
    if mode=="measure":
        ...
        json.dump(pm, open(os.path.join(HERE,"build_pagemap_v2.json"),"w"))
