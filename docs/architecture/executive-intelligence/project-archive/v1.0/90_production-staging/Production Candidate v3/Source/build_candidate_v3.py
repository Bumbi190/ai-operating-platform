...
def from_cover(c):
    ...
    c.drawCentredString(W/2,1.35*72,"PRODUCTION CANDIDATE v3 — NOT FINAL RELEASE")
def from_title(c):
    c.setFillColor(col(DARK)); c.rect(0,0,W,H,fill=1,stroke=0)
    ...
    rows=[("Title","Omnira — Executive Intelligence"),
          ...
          ("Build status","PRODUCTION CANDIDATE v3 — not final professional release"),
...
def set_meta(c):
    c.setTitle("Omnira — Executive Intelligence — Professional Edition Candidate v3 (PRODUCTION CANDIDATE — not final release)")
    c.setAuthor("André Hultgren")
    c.setSubject("Canonical Architecture and Operating Doctrine — Professional Edition PRODUCTION CANDIDATE v3")
    c.setKeywords("Omnira, Executive Intelligence, Professional Edition, PRODUCTION CANDIDATE v3, not final release, Canonical v1.0, Layout C")
    c.setCreator("Omnira Publishing Systems — build_candidate_v3.py (ReportLab)")
...
def main():
    ...
    if mode=="measure":
        ...
        json.dump(pm, open(os.path.join(HERE,"build_pagemap_v3.json"),"w"))
