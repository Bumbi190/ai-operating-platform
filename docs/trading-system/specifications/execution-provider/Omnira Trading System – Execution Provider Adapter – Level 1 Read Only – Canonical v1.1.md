...
**Föregångare:** Execution Provider Adapter – Level 1 Read Only – Canonical v1.0 (låst 2026-08-28, arkiverad)
**Källa:** Canonical Amendments v1.0, Beslut F
...
### F0.1 Vad v1.1 inte ändrar

| Oförändrat från v1.0 |
|---|
| De femton Level 1-metoderna — namn, antal och ansvar |
| Noll order-metoder. `submitOrder` / `modifyOrder` / `cancelOrder` / `preflightOrder` är frånvarande |
| Auktoritetsgränsen (§2). Authority is issued, not derived from data |
...
| `ContractSnapshot` (§7.1) och `HistoryRequest` (§7.2) |
...
| `Available<T>` | Provider Contract | v1.0, oförändrad |
| `CapabilityState` | Provider Contract | v1.0, oförändrad |
| `CredentialMode` | Provider Contract | v1.0, oförändrad |
| `Result<T>` | Provider Contract | NEW — **domänlokal, ej repo-global** |
...
| `ContractSnapshot` · `HistoryRequest` · `FillHistory` | Provider Contract | v1.0, oförändrade |
...
Identiskt med v1.0 §6 i namn, antal och ansvar. Enda skillnaden är att varje refererad typ nu
har en definition. `AccountSnapshot` visas här som `ProviderAccountSnapshot` enligt det
godkända beslutet i F8.1, och är den **enda** signaturändringen i v1.1.
...
interface ExecutionProviderAdapter {
  ...
  getAccountSnapshot(a: AccountId)     : Result<ProviderAccountSnapshot>
...
|---|---|---|---|
| 1 | `connect` | `Result<ProviderSession>` | `Result` F2 · `ProviderError` F2 · `ProviderConfig` F4 · `ProviderSession` F4 · `ProviderId` F14 · `TradingEnvironment` Core · `CredentialMode` v1.0 §4 · `Timestamp` Core |
| 2 | `disconnect` | `void` | — |
| 3 | `getProviderIdentity` | `Result<ProviderIdentity>` | `Result` F2 · `ProviderIdentity` F5 · `ProviderId` F14 · `TradingEnvironment` Core · `Available` v1.0 §5 · `Timestamp` Core |
| 4 | `getEnvironment` | `Result<TradingEnvironment>` | `Result` F2 · `TradingEnvironment` Core |
| 5 | `getCapabilities` | `Result<ProviderCapabilities>` | `Result` F2 · `ProviderCapabilities` v1.0 §3.1 · `CapabilityState` v1.0 §3 · `CredentialMode` v1.0 §4 · `HistoryWindowCapability` **F7** · `Available` v1.0 §5 · `Decimal` Core · `Timestamp` Core |
| 6 | `getHealth` | `Result<ProviderHealth>` | `Result` F2 · `ProviderHealth` **F3** · `Verdict` Core · `ReasonCode` Core · `Timestamp` Core |
| 7 | `getProviderTime` | `Result<ProviderClock>` | `Result` F2 · `ProviderClock` **F6** · `ProviderTimestamp` F6 · `Available` v1.0 §5 · `Decimal` Core · `Timestamp` Core |
| 8 | `getAccounts` | `Result<readonly AccountRef[]>` | `Result` F2 · `AccountRef` **F8.2** · `AccountId` Core · `TradingEnvironment` Core · `Available` v1.0 §5 |
| 9 | `getAccountSnapshot` | `Result<ProviderAccountSnapshot>` | `Result` F2 · `ProviderAccountSnapshot` **F8.3** · `AccountId` Core · `TradingEnvironment` Core · `Available` v1.0 §5 · `Decimal` Core · `Timestamp` Core · `ProviderTimestamp` F6 |
| 10 | `resolveContract` | `Result<ContractRef>` | `Result` F2 · `ContractSpec` **F9** · `ContractRef` **F9** · `InstrumentId` Core · `ContractId` F14 · `Available` v1.0 §5 · `Timestamp` Core |
| 11 | `getContractSnapshot` | `Result<ContractSnapshot>` | `Result` F2 · `ContractSnapshot` v1.0 §7.1 · `ContractId` F14 · `Available` v1.0 §5 · `Decimal` Core · `Timestamp` Core |
| 12 | `getPositions` | `Result<readonly PositionSnapshot[]>` | `Result` F2 · `PositionSnapshot` **F10** · `PositionSide` F10 · `PositionState` F10 · `PositionId` Core · `AccountId` Core · `ContractId` F14 · `InstrumentId` Core · `Available` v1.0 §5 · `Decimal` Core · `Timestamp` Core · `ProviderTimestamp` F6 |
| 13 | `getWorkingOrders` | `Result<readonly OrderSnapshot[]>` | `Result` F2 · `OrderSnapshot` **F11** · `OrderSide` F11 · `OrderType` F11 · `OrderStatus` F11 · `OrderId` Core · `AccountId` Core · `ContractId` F14 · `Available` v1.0 §5 · `Decimal` Core · `Timestamp` Core · `ProviderTimestamp` F6 |
| 14 | `getRecentFills` | `Result<FillHistory>` | `Result` F2 · `HistoryRequest` v1.0 §7.2 · `FillHistory` v1.0 §7.2 · `FillSnapshot` **F11** · `OrderSide` F11 · `FillId` Core · `OrderId` Core · `AccountId` Core · `ContractId` F14 · `Available` v1.0 §5 · `Decimal` Core · `Timestamp` Core · `ProviderTimestamp` F6 |
| 15 | `reconcileReadOnlyState` | `Result<ReadOnlyReconciliation>` | `Result` F2 · `ReadOnlyReconciliation` **F12** · `ReconciliationStatus` F12 · `ReconciliationDiscrepancy` F12 · `DiscrepancyKind` F12 · `ReasonCode` Core · `AccountId` Core · `ContractId` F14 · `PositionId` Core · `OrderId` Core · `Available` v1.0 §5 · `Timestamp` Core |
...
**Summering:** 16 punkter granskade — NO CONFLICT ×9 · CLARIFICATION ×5 ·
INTENTIONAL v1.1 CHANGE ×2 · **UNRESOLVED CONFLICT ×0**. Inga öppna frågor återstår.
...
| Nivå | Level 1 — Read Only |
| Föregångare | Canonical v1.0 (låst 2026-08-28) — arkiverad, oförändrad, historisk hash intakt |
| Beslut | Beslut F — Level-1 Provider Runtime Data Model Completion |
| Slutna specifikationsluckor | 16 av 16 |
| Nya typer | 22 |
| Ändrade v1.0-signaturer | 1 (`getAccountSnapshot` returtyp → `ProviderAccountSnapshot`) |
| Avvisade förslag vid granskning | 3 (`requestedCredentialMode`, `maxPageSize`, `PositionSide.FLAT`) |
| Level 1-metoder | Exakt 15 |
| Order-metoder i Level 1 | Noll |
| Nya reason codes | 2 — `PROVIDER_DISCONNECTED`, `SECURITY_DEGRADED` |
| Olösta konflikter | 0 |
| Öppna frågor | 0 |
| GATE-08 | **Öppen** |
| GATE-16 | Stängd 2026-08-28 |

**Vad denna promotion gör och inte gör**
